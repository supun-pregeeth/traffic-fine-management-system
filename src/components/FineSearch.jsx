  ...
  const handleSubmit = (e) => {
    ...
    fetch(`/api/fines/${refNum}`)
      ...
      .then(body => {
        ...
        if (data && data.categoryId.trim().toUpperCase() === finalCatCode.trim().toUpperCase()) {
          // Map backend response fields to the frontend's fine object model
          const mappedFine = {
            referenceNumber: data.referenceNumber,
            categoryCode: data.categoryId,
            driverName: data.driverName,
            licenseNumber: "B1234567", // Mocked representation since backend uses email
            vehicleNumber: data.vehicleNumber || "WP CAB-4567",
            violation: data.description,
            violationDate: data.issuedAt ? data.issuedAt.substring(0, 10) : new Date().toISOString().substring(0, 10),
            violationTime: data.issuedAt ? data.issuedAt.substring(11, 16) : "12:00",
            location: data.district,
            officerName: data.officerName || "Inspector R. M. Bandara",
            officerBadge: "PS-88452",
            baseAmount: data.amount,
            lateFee: 0,
            dueDate: data.issuedAt ? new Date(new Date(data.issuedAt).getTime() + 14 * 24 * 60 * 60 * 1000).toISOString().substring(0, 10) : new Date().toISOString().substring(0, 10),
            status: data.status.toLowerCase()
          };
          onFineFound(mappedFine);
        } else {
